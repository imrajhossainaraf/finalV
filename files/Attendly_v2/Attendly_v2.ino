...
/**
 * ============================================================
 *  ATTENDLY FIRMWARE v2.3 (Production Ready)
 *  Features: Physical Config Switch, Offline Queue, RTC, NTP Sync
 * ============================================================
 */
...
const long  gmtOffset_sec = 21600; // Bangladesh UTC+6
...
void syncTimeWithNTP() {
  ...
  if (getLocalTime(&timeinfo)) {
    ...
    Serial.println("[Time] RTC Synchronized with NTP");
  } else {
...
void saveToQueue(String uid, String timestamp) {
  ...
  file.close();
  Serial.printf("[SPIFFS] Saved to queue: %s\n", uid.c_str());
}
...
void syncLocalData() {
  ...
  if (code >= 200 && code < 300) {
    ...
    Serial.printf("[Sync] Failed (Code %d). Queue preserved.\n", code);
  }
  http.end();
}

// ================= REAL-TIME POST =================
bool postRealTime(String uid, String timestamp) {
  ...
  if (success) {
    ...
    Serial.printf("[HTTP] Real-time failed (%d).\n", code);
  }
  http.end();
  return success;
}
...
void handleSave() {
  ...
  if (server.hasArg("password") && server.arg("password").length() > 0) configPass = server.arg("password");
...
void setup() {
  ...
  if (rtc.begin()) {
    ...
    if (rtc.lostPower()) {
      Serial.println("[RTC] Power lost, sync required.");
    }
  } else {
    beepError();
  }
  ...
  if (digitalRead(BUTTON_PIN) == LOW && configSSID != "") {
    ...
    if (WiFi.status() == WL_CONNECTED) {
      Serial.println("\n[WiFi] Connected!");
      beepSuccess();
      syncTimeWithNTP(); // Sync RTC on boot
    } else {
...
void loop() {
  ...
  // Periodic Sync & Time Sync (Only if WiFi connected)
  ...
  if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
    ...
    // Attempt Real-time Post First
    if (!postRealTime(uid, timestamp)) {
