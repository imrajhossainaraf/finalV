...
app.post("/api/chat", async (req, res) => {
  ...
  if (provider !== "local" && !apiKey) {
    return res
      .status(400)
      .json({ error: "API Key is missing for external provider." });
  }
  ...
  try {
    ...
    if (provider === "local") {
      ...
      if (lowerReq.includes("hello") || lowerReq.includes("hi")) {
        responseText =
          'Hello! I am your Local Offline Assistant. I do not use external APIs. Try saying "send a bulk notice" or "email [Student Name]".';
      } else if (
        lowerReq.includes("bulk") ||
        lowerReq.includes("everyone") ||
        lowerReq.includes("all")
      ) {
        responseText =
          "I will send a bulk notification to all students based on today's records.";
        actionStr = '@@ACTION:{"type": "bulk_notice"}@@';
      } else if (
        lowerReq.includes("email") ||
        lowerReq.includes("send") ||
        lowerReq.includes("notice")
      ) {
        ...
        responseText =
          "I am currently in Offline Local Mode. I only understand basic keywords like 'bulk notice' or 'send email to [Name]'. Please try rephrasing.";
      }
